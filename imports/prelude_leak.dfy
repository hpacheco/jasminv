...
predicate ValidAddress(i:int)

trait Mem
{
  var memory: array<bv8>;

  predicate Valid(i: int)
  reads this, memory;
  {
    this.memory != null && this.memory.Length > i >= 0 && ValidAddress(i)
  }
  predicate ValidRange(i:int, j:int)
  reads this, memory;
  {
    forall k :: i <= k <= j ==> Valid(k)
  }
